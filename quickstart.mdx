...
<Steps>
  <Step title="Set your API key">
    ...
    Your operator at Teel issues your key via the admin flow described in the [sandbox onboarding runbook](https://github.com/TeelOrg/teel-backend/blob/main/docs/superpowers/runbooks/sandbox-partner-onboarding.md) and shares it via 1Password / encrypted channel. The plaintext is shown **exactly once** at creation; afterwards only the first 12 characters are retrievable.
    ...
    <Note>
      If `api-sandbox.teel.finance` doesn't resolve yet on your network, `api-dev.teel.finance` is a transitional alias accepting the same `sk_test_…` keys while DNS propagates.
    </Note>
  </Step>
  ...
  <Step title="Get a quote">
    Fetch the best rate across all providers for your corridor. Quotes are short-lived (~minutes) and carry an opaque `protocol` token you'll pass back at execution time to pin the route.
...
Every endpoint above is documented in the machine-readable spec at [`/openapi.json`](https://api-sandbox.teel.finance/openapi.json) (or `/openapi.yaml`). Import it into Postman, Insomnia, or run `openapi-generator-cli` for a typed client in any language.
