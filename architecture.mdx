...
Each partner runs their own independent compliance review under their license. Teel surfaces the per-partner approval state via [`GET /kyb/onboarding-status`](/api-reference/kyb/onboarding-status), and routes transactions only to partners that have approved your KYB for the relevant corridor.
