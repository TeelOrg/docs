...
The partner API supports two payout types — a **Payout** and a **Stablecoin payout**. Each is settled internally through one or more legs — **collection**, **on-chain escrow**, and **disbursement** — every leg executed by a licensed partner. The legs and the escrow are settlement plumbing, not products you opt into.

### Payout

The most common pattern. Your business sends fiat from your bank account; the recipient receives fiat in theirs. Settled through a collection leg, an on-chain escrow hop, and a disbursement leg.
...
sequenceDiagram
    ...
    participant POn as Collection partner<br/>(licensed EMI)
    participant Esc as Smart-wallet escrow
    participant POff as Disbursement partner<br/>(licensed in destination)
    participant Rec as Recipient bank

    You->>Teel: POST /rfq/execute (routeProtocol)
    ...
    Esc->>POff: Release stablecoin to disbursement partner
    POff->>Rec: Pay out destination fiat (e.g. PHP)
    POff-->>Teel: Settlement confirmation
    Teel-->>You: payout.status.updated webhook (status: delivered)
```

Two licensed partners are involved per transaction — one regulated to collect the source fiat, one regulated to disburse the destination fiat. The stablecoin step in the middle exists purely as a settlement primitive between the two regulated legs.

### Stablecoin payout

The recipient receives a stablecoin balance on-chain instead of fiat. Settled through the collection leg only — the first half of the diagram above, with the disbursement leg skipped.

### Stablecoin-to-stablecoin (dashboard)

You fund the payout with stablecoin from a wallet you control, and the recipient receives stablecoin — potentially on a different chain. Settled on-chain via DEX swap and/or a cross-chain bridge; no fiat collection or disbursement legs are involved. This is a **dashboard feature** today — it isn't exposed on the partner API. See [Crypto payouts](/guides/crypto-payout).
...
flowchart LR
    ...
    Status["Per-partner approval status<br/>(visible in the dashboard)"]
