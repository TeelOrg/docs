...
Lookup by the `quoteId` returned from [`POST /rfq/execute`](/api-reference/payouts/execute). Response is wrapped: `{ "success": true, "data": <QuoteStatus> }`. For push-based updates, subscribe to `payout.status.updated` and `payout.delivered` via [Webhooks](/guides/webhooks) instead of polling.
